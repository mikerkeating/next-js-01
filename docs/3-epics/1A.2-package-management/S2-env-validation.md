...
| ------------------------ | ----------------------------------------------------- | -------------------------------------------- |
| `server` schema          | Define all `process.env` variables (non-NEXT*PUBLIC*) | [t3-env docs](https://env.t3.gg/docs/nextjs) |
...
- **Server vs Client**: Server variables are only available server-side; client variables (NEXT*PUBLIC*) are bundled into client code

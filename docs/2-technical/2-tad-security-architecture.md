...
async function generateExport(userId: string, options: ExportOptions) {
  ...
  const blob = await put(`exports/${userId}-${Date.now()}.${options.format}`, fileBuffer, {
    access: "public",
  ...
  return async (req: Request, res: Response, next: NextFunction) => {
    ...
    res.json = function (body: any) {
      ...
      if (shouldAudit(req.method, req.path, res.statusCode)) {
        ...
        auditLogger
          .log(auditEntry, getRequestContext(req))
...
export function withAudit<T extends AnyTable>(table: T, operation: "insert" | "update" | "delete") {
  return {
...
export async function cleanupAuditLogs() {
  ...
  for (const [severity, retentionDays] of Object.entries(RETENTION_POLICIES)) {
    ...
    const logsToArchive = await db
      .select()
      .from(audit_logs)
      .where(and(eq(audit_logs.severity, severity), lt(audit_logs.timestamp, cutoffDate)))
...
export const auditQueries = {
  ...
  async getResourceHistory(resourceType: string, resourceId: string) {
    return await db
      ...
      .from(audit_logs)
      .where(
        and(eq(audit_logs.resource_type, resourceType), eq(audit_logs.resource_id, resourceId))
      )
      .orderBy(desc(audit_logs.timestamp));
  },
  ...
  async getCriticalEvents(since: Date) {
    return await db
      .select()
      .from(audit_logs)
      .where(and(eq(audit_logs.severity, "critical"), gte(audit_logs.timestamp, since)))
      .orderBy(desc(audit_logs.timestamp));
  },
...
router.post("/api/v1/audit/export", requireRole("internal"), async (req, res) => {
  ...
  const logs = await auditQueries.getOrgAuditTrail(orgId, new Date(startDate), new Date(endDate));
  ...
  res.setHeader(
    "Content-Disposition",
    `attachment; filename="audit-log-${orgId}-${Date.now()}.${format}"`
  );
  res.send(exportData);
});
```
...
export async function monitorCriticalEvents() {
  ...
  const criticalEvents = await db
    .select()
    .from(audit_logs)
    .where(and(eq(audit_logs.severity, "critical"), gte(audit_logs.timestamp, fiveMinutesAgo)));
  ...
  const permChanges = criticalEvents.filter(
    (e) => e.action.includes("permission") || e.action.includes("role")
  );
...
async function checkRateLimit(key: string, limit: number, windowMs: number): Promise<boolean> {
  ...
  let bucket = await db.query.rate_limit_buckets.findFirst({
    where: and(eq(rate_limit_buckets.key, key), gte(rate_limit_buckets.window_start, windowStart)),
  });
...
export async function cleanupRateLimits() {
  ...
  await db.delete(rate_limit_buckets).where(lt(rate_limit_buckets.expires_at, now));
}
```
...
export class EncryptionService {
  ...
  async encrypt(plaintext: string): Promise<string> {
    ...
    const key = (await scryptAsync(process.env.ENCRYPTION_KEY, salt, this.keyLength)) as Buffer;
...
export async function rotateEncryptionKey(newKey: string) {
  ...
  for (const key of apiKeys) {
    ...
    await db.update(api_keys).set({ key_encrypted: reencrypted }).where(eq(api_keys.id, key.id));
  }

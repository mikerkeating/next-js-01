...
- **GitHub Actions natively supports skipping CI via commit messages**: Include `[skip ci]`, `[ci skip]`, `[no ci]`, `[skip actions]`, or `[actions skip]` in a commit message to skip workflow runs. This only works for `push` events (not `pull_request`), and only the head commit message is checked when pushing multiple commits. Useful for documentation-only changes to avoid wasting CI minutes

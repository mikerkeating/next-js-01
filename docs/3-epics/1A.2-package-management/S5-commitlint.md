...
- [x] Commitlint is installed at the version specified in [canonical-versions.md](/docs/2-technical/references/canonical-versions.md)
- [x] Commits following [Conventional Commits 1.0.0](https://www.conventionalcommits.org/) format are accepted
- [x] Commits not following conventional format are rejected with helpful error messages
- [x] The `commit-msg` Husky hook runs commitlint on every commit
- [x] Standard types (`feat`, `fix`, `docs`, `style`, `refactor`, `test`, `chore`) are supported
- [x] Scopes are optional but encouraged (e.g., `feat(auth):`, `fix(api):`)
- [x] Breaking changes are supported via `!` suffix or `BREAKING CHANGE:` footer
- [x] Developers can bypass with `git commit --no-verify` for emergencies
...
- [x] Commitlint version matches canonical-versions.md
- [x] Valid conventional commits accepted
- [x] Invalid commits rejected with helpful messages
- [x] Breaking change syntax works

### Documentation

- [x] Config file includes comment explaining preset choice

### Git Hygiene

- [x] Conventional commit message used for this story
...
Installed and configured commitlint with the `@commitlint/config-conventional` preset to enforce Conventional Commits format on all commit messages. The `.husky/commit-msg` hook now validates commit messages automatically, rejecting non-compliant commits with helpful error messages.
...
| Valid commit    | `echo "feat: test" \| pnpm exec commitlint`      | Pass                            |
| Invalid commit  | `echo "invalid" \| pnpm exec commitlint`         | Rejected with errors            |
| Breaking change | `echo "feat!: breaking" \| pnpm exec commitlint` | Pass                            |

### Files Changed

All files are as specified in the story:

- `commitlint.config.js` - Created with conventional preset
...
- Shell escaping can interfere with `!` in commit messages when testing via echo/pipe; using heredocs avoids this issue
